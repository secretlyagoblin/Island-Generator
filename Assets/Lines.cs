    ...
    */
    ...
    void DoShortest()
    {
        ...
        var finalMap = Map.BlankMap(size, size)
            ...
            .ApplyMap(bottomRight, new Coord((int)(size * 0.5f), (int)(size * 0.5f)))
            //.Display()
            .Add(Map.BlankMap(size,size).PerlinFill(size * 0.15f, 1, 0, seed).Remap(-0.25f, 0.25f))
            .Clamp(-0.25f, 1)
            .Normalise()
            .Remap(MyCurve);
        //.Display()
        ...
        for (int i = 0; i < smartmesh.Lines.Count; i++)
        {
            ...
            var walk = smartmesh.ShortestWalkNode(RNG.Next(smartmesh.Nodes.Count), RNG.Next(smartmesh.Nodes.Count));

        for (int i = 0; i < walk.Length-1; i++)
        {
            Debug.DrawLine(smartmesh.Nodes[walk[i]].Vert, smartmesh.Nodes[walk[i + 1]].Vert,Color.red,100f);
        }
    }
